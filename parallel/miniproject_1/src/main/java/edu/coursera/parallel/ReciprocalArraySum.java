...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        final int SEQUENTIAL_THRESHOLD = 10000;
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        // Compute sum of reciprocals of array elements
        System.setProperty("java.until.concurrent.ForkJoinPool.common.parallelism", String.valueOf(numTasks));

        List<ReciprocalArraySumTask> output = new ArrayList<>();
        for (int i=0; i < numTasks; i++){
            output.add(new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length),
                                                   getChunkEndExclusive(i, numTasks, input.length), input));
        }
        for (int i=1; i<numTasks; i++){
            output.get(i).fork();
        }
        output.get(0).compute();
        for (int i=1; i<numTasks; i++){
            output.get(i).join();
        }
        for(ReciprocalArraySumTask task : output){
            sum += task.getValue();
        }
        return sum;
    }
